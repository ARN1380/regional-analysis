...
import { 
  ...
  Newspaper, Radio, AlertTriangle, Play, Pause, Download, Loader2 
} from 'lucide-react';
import { 
...
const Header = ({ appLanguage, setAppLanguage, uiTranslations, currentTimeState }) => {
  ...
  return (
    <header className="flex flex-col items-center justify-center pt-8 pb-6 relative">
      {/* Added no-capture class here to hide from html-to-image */}
      <div className="flex bg-zinc-800 rounded-lg p-1 mb-6 gap-2 no-capture">
        <button onClick={() => setAppLanguage('fa')} className={`px-6 py-2 rounded-md text-sm font-medium transition-colors ${isPersian ? 'bg-blue-600 text-white' : 'text-zinc-400 hover:text-white'}`}>فارسی</button>
        <button onClick={() => setAppLanguage('en')} className={`px-6 py-2 rounded-md text-sm font-medium transition-colors ${!isPersian ? 'bg-blue-600 text-white' : 'text-zinc-400 hover:text-white'}`}>English</button>
      </div>
...
const MetricCard = ({ title, value, isFloatValue = false, baseValue, percentageChange, isTrendingUp, uiTranslations, prefix = "", suffix = "", subtitle = "" }) => (
  <div className="bg-[#1e1e24] p-5 rounded-xl border border-zinc-800 flex flex-col justify-between h-36 hover:border-zinc-700 transition-colors shadow-sm">
    <div className="flex justify-between items-start">
      <div>
        ...
        {/* Render the subtitle (TEU description) if provided, otherwise the default compare text */}
        {subtitle ? (
      ...
      <div className="w-6 h-6 rounded bg-blue-500 flex items-center justify-center shadow-lg shadow-blue-500/20">
      </div>
    </div>
        ...
        {dailyMetrics.news.map((newsItem) => {
          ...
          return (
            <div 
              ...
              className={`border-l-2 pl-3 rtl:border-r-2 rtl:border-l-0 rtl:pr-3 rtl:pl-0 cursor-pointer p-2 transition-all rounded-r-lg rtl:rounded-l-lg rtl:rounded-r-none ${isActive ? 'border-blue-500 bg-zinc-800/80 shadow-md' : 'border-zinc-700 hover:border-blue-400 hover:bg-zinc-800/30'}`}
            >
...
const EnergyMarketCard = ({ appLanguage, uiTranslations, dailyMetrics, chartData }) => {
  ...
  return (
    <div className="bg-[#1e1e24] p-5 rounded-xl border border-zinc-800 flex flex-col h-full">
      <h2 className="text-white font-semibold mb-4 flex items-center gap-2"><BarChart3 className="text-yellow-500" size={18} /> {uiTranslations.energyStatus}</h2>
      <div className="bg-zinc-900/50 rounded-lg h-56 mb-6 shrink-0 border border-zinc-800/80 pt-4 pr-4 pb-2" dir="ltr">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
            ...
            <Line type="monotone" dataKey="brent" stroke="#ef4444" strokeWidth={2} dot={{ r: 3, fill: '#ef4444', strokeWidth: 0 }} activeDot={{ r: 5, strokeWidth: 0 }} animationDuration={800} />
            <Line type="monotone" dataKey="lng" stroke="#eab308" strokeWidth={2} dot={{ r: 3, fill: '#eab308', strokeWidth: 0 }} activeDot={{ r: 5, strokeWidth: 0 }} animationDuration={800} />
            <Line type="monotone" dataKey="gasoline" stroke="#3b82f6" strokeWidth={2} dot={{ r: 3, fill: '#3b82f6', strokeWidth: 0 }} activeDot={{ r: 5, strokeWidth: 0 }} animationDuration={800} />
          </LineChart>
        </ResponsiveContainer>
      </div>
...
const MarineTrafficCard = ({ uiTranslations, dailyMetrics }) => (
  <div className="bg-[#1e1e24] p-5 rounded-xl border border-zinc-800 flex flex-col h-full">
    <h2 className="text-white font-semibold mb-4 flex items-center gap-2"><Ship className="text-blue-500" size={18} /> {uiTranslations.hormuzTraffic}</h2>
    
    {/* REPLACED LEAFLET MAP WITH STATIC IMAGE */}
    <div className="bg-zinc-900 rounded-lg h-56 mb-6 shrink-0 relative overflow-hidden border border-zinc-800 z-0 flex items-center justify-center">
       {/* Make sure you have an image named "marine-map.jpg" inside your Next.js "public" folder. 
         The gradient overlay makes it match the dark UI perfectly. 
       */}
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
      ...
      <div className="bg-[#1e1e24] p-5 rounded-xl border border-zinc-800">
        ...
        <div className="flex justify-between items-end mb-6 pb-4 border-b border-zinc-800/50">
          <div><p className="text-zinc-500 text-xs mb-1">{uiTranslations.gatheringsToday}</p><p className="text-zinc-600 text-[10px]">{uiTranslations.base} 0</p></div>
          <div className="text-end">
      ...
      <div className="bg-[#1e1e24] p-5 rounded-xl border border-zinc-800 flex flex-col justify-between">
        ...
        <div className="flex justify-between items-center mb-6 px-1">
          <div><p className="text-zinc-300 text-sm">{uiTranslations.womenChildren}</p><p className="text-zinc-500 text-[11px] mt-0.5">{uiTranslations.ofTotal}</p></div>
          <div className="text-end">
  ...
  return (
    <div className="mt-6 space-y-4">
      ...
      <div className="bg-[#1e1e24] p-4 rounded-xl border border-zinc-800 flex justify-between items-center">
        ...
        {/* Added no-capture here so the button itself hides from the screenshot */}
        <button onClick={handleDownload} disabled={isDownloading} className={`bg-blue-600 text-white px-5 py-2.5 rounded-lg text-sm font-medium flex items-center gap-2 transition-all no-capture ${isDownloading ? 'opacity-60 cursor-not-allowed' : 'hover:bg-blue-500 hover:shadow-lg hover:shadow-blue-900/20 active:scale-95'}`}>
...
export default function RegionalDashboard() {
  ...
  useEffect(() => { setCurrentTimeState(new Date()); const timer = setInterval(() => setCurrentTimeState(new Date()), 60000); return () => clearInterval(timer); }, []);
  ...
  // Custom Filter to absolutely hide the language toggle and download button
  ...
  const handleDownload = async () => { 
    ...
    try { 
      ...
      const dataUrl = await toPng(dashboardRef.current, { 
        ...
        filter: screenshotFilter // Applied filter to hide specific DOM nodes
      }); 
  ...
  const energyChartData = useMemo(() => { const data = []; for (let i = 1; i <= selectedDay; i++) { const historicalDay = dashboardData.days[i.toString()]; if (historicalDay) { data.push({ name: appLanguage === 'en' ? `Day ${i}` : `روز ${i}`, brent: historicalDay.energy.brent.price, lng: historicalDay.energy.lng.price, gasoline: historicalDay.energy.gasoline.price }); } } return data; }, [selectedDay, appLanguage]);
    ...
    <div ref={dashboardRef} dir={layoutDirection} className={`min-h-screen bg-[#121212] selection:bg-blue-500/30 text-zinc-100 p-4 md:p-8 ${appLanguage === 'en' ? inter.className : vazirmatn.className}`}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
          <MetricCard title={uiTranslations.brentOil} value={dailyMetrics.kpis.brent.value} isFloatValue={true} baseValue={dailyMetrics.kpis.brent.base} percentageChange={dailyMetrics.kpis.brent.change} isTrendingUp={dailyMetrics.kpis.brent.isUp} uiTranslations={uiTranslations} prefix="$" />
          ...
          {/* Added subtitle for the TEU explanation here */}
          <MetricCard title={"TEU Index"} subtitle={uiTranslations.teuDesc} value={dailyMetrics.kpis.teu.value} isFloatValue={false} baseValue={dailyMetrics.kpis.teu.base} percentageChange={dailyMetrics.kpis.teu.change} isTrendingUp={dailyMetrics.kpis.teu.isUp} uiTranslations={uiTranslations} />
        </div>
